        ...
        initThreeJS() {
            ...
            const loadModel = (path, position, targetSize, rotation, name) => {
                gltfLoader.load(path, (gltf) => {
                    ...
                    // Updates the model's world matrix (first argument is the update flag for whether the matrix should be updated, second is for child objects)
                    model.updateWorldMatrix(true, true);

                    // Create a new Box3 (axis-aligned bounding box) from the object's geometry in world space
                    const worldBox = new THREE.Box3().setFromObject(model);

                    // Invert the model's world matrix to convert from world space to local space
                    const inv = new THREE.Matrix4().copy(model.matrixWorld).invert();

                    // Apply the inverted matrix to the world box to convert it from world space to local space
                    model.userData.localBox = worldBox.clone().applyMatrix4(inv); // Store the local box in the model's user data

                    // Create an empty Box3 to hold the world box, which will be updated later
                    model.userData.worldBox = new THREE.Box3(); // Holder for world box

                    // Calculate the size and center of the model's bounding box in local space
                    const lb = model.userData.localBox; // Access the model's local bounding box
                    const sizeL = new THREE.Vector3().subVectors(lb.max, lb.min); // Compute the size of the bounding box
                    const centerL = new THREE.Vector3().addVectors(lb.min, lb.max).multiplyScalar(0.5); // Find the center of the bounding box

                    // Create a geometry for the pick proxy (invisible box used for raycasting) using the calculated size
                    const pickGeo = new THREE.BoxGeometry(sizeL.x, sizeL.y, sizeL.z);

                    // Create a material for the pick proxy (set to invisible to not render it)
                    const pickMat = new THREE.MeshBasicMaterial({ visible: false });

                    // Create the pick proxy mesh using the geometry and material
                    const pickProxy = new THREE.Mesh(pickGeo, pickMat);

                    // Set the position of the pick proxy to the center of the model's bounding box
                    pickProxy.position.copy(centerL);

                    // Set the name of the pick proxy to help identify it, adding a "_pickProxy" suffix
                    ...
                    // Add the pick proxy to the array of targets for raycasting
        ...
        onPointerMove(event) {
            ...
            if (this.isDragging && this.dragObject) {
                ...
                if (this.raycaster.ray.intersectPlane(this.dragPlane, intersection)) {
                    ...
                    if (this.intersectsAny(obj)) {  // Check if the object intersects with any other object
                        // Revert the object’s position to its original values if an intersection is found
                        obj.position.x = oldX;
                        obj.position.z = oldZ;

                        // Calculate the difference (dx, dz) between the clamped position and the original position
                        const dx = clampedX - oldX, dz = clampedZ - oldZ;
                        
                        // Calculate the distance between the original position and the clamped position using the Euclidean distance formula
                        const len = Math.hypot(dx, dz);

                        if (len > 1e-6) {  // If the distance is significant (greater than a small threshold)
                            const eps = 0.5;  // A small value used to slightly nudge the object and avoid sticking
                            // Calculate the new nudge position, reducing the distance to avoid intersection
                            const nx = oldX + (dx / len) * Math.max(0, len - eps);
                            const nz = oldZ + (dz / len) * Math.max(0, len - eps);

                            // Update the object’s position with the new nudge position
                            obj.position.x = nx;
                            obj.position.z = nz;

                            // Check if the object still intersects with any other object after nudging
                            if (this.intersectsAny(obj)) {
                                // If it still intersects, revert the object’s position back to the original values
                                obj.position.x = oldX;
                                obj.position.z = oldZ;
                            }
                        }
